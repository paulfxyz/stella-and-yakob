#!/usr/bin/env python3
"""
compose_v44.py — Compositor for Stella & Yakob v4.4
=====================================================
Page order per PDF:
...
OUT_DIR = f"{WS}/book_pages_{LANG}_v44"
...
def make_dedication():
    ...
    # Tiny balloon art — placed in upper 55% of page
    art_src = f"{WS}/dedication_art.png"
    if os.path.exists(art_src):
        art = Image.open(art_src).convert("RGBA")
        # Scale to fit in a 900×900 zone centred at top
        art_max = 900
        aw, ah  = art.size
        art_scale = min(art_max/aw, art_max/ah)
        nw, nh  = int(aw*art_scale), int(ah*art_scale)
        art_r   = art.resize((nw, nh), Image.LANCZOS)
        x = (PX - nw) // 2
        y = 120
        pg.paste(art_r, (x, y), art_r)
        text_start_y = y + nh + 80
    else:
        text_start_y = PX // 2 - 200

    # Quote text — wrap to ~36 chars per line for display
    import textwrap
    # For RTL languages, we just centre as-is
    max_chars = 38
    wrapped = textwrap.fill(quote, width=max_chars)
    quote_lines = wrapped.split("\n")

    # Measure total quote block
    q_h = len(quote_lines) * (font_quote.getbbox("Ag")[3] - font_quote.getbbox("Ag")[1] + 20)
    attr_h = font_quote_attr.getbbox("Ag")[3] - font_quote_attr.getbbox("Ag")[1] + 16

    # Centre block vertically in remaining space
    remaining = PX - text_start_y - 120
    total_text_h = q_h + 50 + attr_h
    y = text_start_y + (remaining - total_text_h) // 2
    y = max(y, text_start_y)

    q_line_h = font_quote.getbbox("Ag")[3] - font_quote.getbbox("Ag")[1] + 22
    for line in quote_lines:
        tw = font_quote.getbbox(line)[2] - font_quote.getbbox(line)[0]
        x  = (PX - tw) // 2
...
def make_qr_page():
    ...
    qr_src = f"{WS}/qr_sandy.png"
    url    = "sandy.page"

    if os.path.exists(qr_src):
        qr  = PILImage.open(qr_src).convert("RGBA")
        # Scale QR to 700×700 px, centred slightly above middle
        qr_sz = 700
        qw, qh = qr.size
        qs = min(qr_sz/qw, qr_sz/qh)
        nw, nh = int(qw*qs), int(qh*qs)
        qr_r = qr.resize((nw, nh), Image.LANCZOS)
        x = (PX - nw) // 2
        y = (PX - nh) // 2 - 100
        pg.paste(qr_r, (x, y), qr_r)
        url_y = y + nh + 60
    else:
        url_y = PX // 2 + 200
    ...
    draw.text(((PX-tw)//2, url_y), url, font=font_url, fill=MUTED_INK)
...
out_pdf = os.path.join(WS, f"{fname}_v44.pdf")
